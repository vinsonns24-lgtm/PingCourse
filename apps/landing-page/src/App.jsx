...
function App() {
  return (
    <>
      {/* TopAppBar (Web) */}
      <header className="hidden md:flex justify-between items-center px-lg h-16 w-full fixed top-0 z-50 bg-surface/70 dark:bg-surface-dim/70 backdrop-blur-xl border-b border-white/10 shadow-[0px_8px_24px_rgba(0,0,0,0.04)]">
        ...
        <div className="flex items-center gap-sm">
          <button className="font-label-md text-label-md text-primary px-4 py-2 hover:bg-surface-variant/50 rounded-lg transition-colors">Sign In</button>
          <button className="bg-primary text-on-primary font-label-md text-label-md px-4 py-2 rounded-lg shadow-[0_2px_0_0_rgba(0,0,0,0.1)] hover:bg-primary/90 transition-colors active:scale-95 duration-200">Get Started</button>
        </div>
      </header>

      {/* TopAppBar (Mobile) */}
      <header className="md:hidden flex justify-between items-center px-margin-mobile h-16 w-full fixed top-0 z-50 bg-surface/70 dark:bg-surface-dim/70 backdrop-blur-xl border-b border-white/10 shadow-[0px_8px_24px_rgba(0,0,0,0.04)]">
        <div className="flex items-center gap-sm">
        ...
        <img alt="User profile" className="w-8 h-8 rounded-full border border-outline-variant cursor-pointer active:scale-95 transition-transform duration-200" data-alt="A small, professional headshot of a user smiling warmly, used as an avatar in a modern web application interface. The lighting is bright and clear." src="https://lh3.googleusercontent.com/aida-public/AB6AXuAtdEOIKy0yEzjHV5u3ypQvIu96AKF-1XqF8GNm_pyeO8FCY0x9FbJYqtK_SDni4VVNcrLZIMUR94cfZQbIzSDrKmvfiiYEgP3Loi9fLYKYNRZwOLFnG44-rbGcbV35zTM1Jrac9gPj6S-JVB7vQujvWRTRhHn8tDSg2CQrX0shUmBT1mJJDykEQFNq_SG_nxGmA0rYRpQO3Ew5wouuSJjREQf_qDHM4WqXJmkhaaX8diBWT9bL64CXX9TE5dDRkQeesjj_Bgy0bok" />
      </header>

      <main className="flex-grow flex flex-col md:pl-0 md:pt-16 pb-24 md:pb-0">
        {/* Hero Section */}
        <section className="relative w-full max-w-[1280px] mx-auto px-margin-mobile md:px-lg py-xl flex flex-col lg:flex-row items-center gap-xl min-h-[819px] bg-pattern">
          <div className="flex-1 flex flex-col items-start z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-sm w-full sm:w-auto">
              <button className="bg-[#25D366] text-white font-label-md text-label-md px-6 py-3 rounded-lg shadow-[0_2px_0_0_rgba(0,0,0,0.1)] hover:bg-[#20bd5a] transition-colors flex items-center justify-center gap-sm active:scale-95 duration-200">
                <span className="material-symbols-outlined">chat</span> Daftar via WhatsApp
              </button>
              <button className="bg-surface/70 text-primary font-label-md text-label-md px-6 py-3 rounded-lg border-[1.5px] border-primary/20 hover:bg-primary/10 transition-colors flex items-center justify-center gap-sm active:scale-95 duration-200 glass-card">
                Take Placement Test
              </button>
            </div>
          </div>
          <div className="flex-1 w-full relative z-10 flex justify-center lg:justify-end">
            <div className="relative w-full max-w-md aspect-[4/5] rounded-2xl overflow-hidden shadow-2xl glass-card p-2 border-white/40">
              <img alt="Students studying together" className="w-full h-full object-cover rounded-xl" data-alt="A vibrant image showing a diverse group of ambitious students collaborating around a sleek modern table in a well-lit, contemporary library or study space. The atmosphere is focused yet engaging, typical of a high-performance educational setting. Sunlight streams through large windows, creating a bright, optimistic light-mode aesthetic." src="https://lh3.googleusercontent.com/aida-public/AB6AXuAc1q6gjw1g_OUGYSfXfQ2mWqd1a8WGF0WV6IxxZ8C5Iw9oRlJftJO_CX_WBjWfn3Qj0-bF5DIW1pZEZ0u0-pFSzR-ZOS34W8_iyUCpd1a8rwVRzpX4MiRzPNL2T-8Ft74uK3GIUHB1_x8tjXnV8IOtEkGeHdYitYQtxos7jCrbMeoZFxxLkVhvO_Ecm3aImMmPd6P37UzIGV7HU3glQwiFETNjumMsyVXCwT05d01FaKwEq0LmYWC-3MObhyZhDq6qam72TvmTcyQ" />
              {/* Floating Badge */}
              <div className="absolute bottom-6 left-6 right-6 glass-card rounded-xl p-4 flex items-center gap-sm animate-pulse-slow">
        ...
        <section className="w-full max-w-[1280px] mx-auto px-margin-mobile md:px-lg py-xl" id="programs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
            ...
            <div className="glass-card rounded-2xl p-md flex flex-col border-l-4 border-l-primary hover:-translate-y-1 transition-transform duration-300">
            ...
            <div className="glass-card rounded-2xl p-md flex flex-col border-l-4 border-l-secondary hover:-translate-y-1 transition-transform duration-300">
      ...
      {/* BottomNavBar (Mobile) */}
      <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 py-3 pb-safe bg-surface/80 dark:bg-surface-container-highest/80 backdrop-blur-xl rounded-t-xl border-t border-white/10 shadow-[0px_-4px_16px_rgba(0,0,0,0.04)] shadow-lg">
